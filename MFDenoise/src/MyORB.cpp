 ...
 */
static void HarrisResponses(const Mat& img, const std::vector<Rect>& layerinfo,
...
static void
    computeOrbDescriptors( const Mat& imagePyramid, const std::vector<Rect>& layerInfo,
                           ...
                           Mat& descriptors, const std::vector<Point>& _pattern, int dsize, int wta_k,float _iso )
{
    ...
    for( j = 0; j < nkeypoints; j++ )
    {
        ...
        // sprintf(path, "/data/local/%d.jpg", *center);
        // imwrite(path,imagePyramid);
        // LOGE("center = %d,x = %d,y =%d\n",*center,cvRound(kpt.pt.y*scale) + layer.y,cvRound(kpt.pt.x*scale) + layer.x);
        float x, y;
        int ix, iy;
        ...
        *(center + iy*step + ix) )
        ...
        if( wta_k == 2 )
        {
            for (i = 0; i < dsize; ++i, pattern += 16)
            {
                ...
                {
                    int t0, t1, val;
                    t0 = GET_VALUE_BLOCK(0); t1 = GET_VALUE_BLOCK(1);
                    val = t0 < t1;
                    t0 = GET_VALUE_BLOCK(2); t1 = GET_VALUE_BLOCK(3);
                    val |= (t0 < t1) << 1;
                    t0 = GET_VALUE_BLOCK(4); t1 = GET_VALUE_BLOCK(5);
                    val |= (t0 < t1) << 2;
                    t0 = GET_VALUE_BLOCK(6); t1 = GET_VALUE_BLOCK(7);
                    val |= (t0 < t1) << 3;
                    t0 = GET_VALUE_BLOCK(8); t1 = GET_VALUE_BLOCK(9);
                    val |= (t0 < t1) << 4;
                    t0 = GET_VALUE_BLOCK(10); t1 = GET_VALUE_BLOCK(11);
                    val |= (t0 < t1) << 5;
                    t0 = GET_VALUE_BLOCK(12); t1 = GET_VALUE_BLOCK(13);
                    val |= (t0 < t1) << 6;
                    t0 = GET_VALUE_BLOCK(14); t1 = GET_VALUE_BLOCK(15);
                    val |= (t0 < t1) << 7;

                    desc[i] = (uchar)val;
                }else {
                    int t0, t1, val;
                    t0 = GET_VALUE_PIXEL(0); t1 = GET_VALUE_PIXEL(1);
                    val = t0 < t1;
                    t0 = GET_VALUE_PIXEL(2); t1 = GET_VALUE_PIXEL(3);
                    val |= (t0 < t1) << 1;
                    t0 = GET_VALUE_PIXEL(4); t1 = GET_VALUE_PIXEL(5);
                    val |= (t0 < t1) << 2;
                    t0 = GET_VALUE_PIXEL(6); t1 = GET_VALUE_PIXEL(7);
                    val |= (t0 < t1) << 3;
                    t0 = GET_VALUE_PIXEL(8); t1 = GET_VALUE_PIXEL(9);
                    val |= (t0 < t1) << 4;
                    t0 = GET_VALUE_PIXEL(10); t1 = GET_VALUE_PIXEL(11);
                    val |= (t0 < t1) << 5;
                    t0 = GET_VALUE_PIXEL(12); t1 = GET_VALUE_PIXEL(13);
                    val |= (t0 < t1) << 6;
                    t0 = GET_VALUE_PIXEL(14); t1 = GET_VALUE_PIXEL(15);
                    val |= (t0 < t1) << 7;
...
int MyORB::descriptorSize() const
{
    return kBytes;
}

int MyORB::descriptorType() const
{
    return CV_8U;
}

int MyORB::defaultNorm() const
{
    return NORM_HAMMING;
}
                             ...
                             bool useOCL, int fastThreshold  )
{
    ...
    for( level = 0; level < nlevels; level++ )
    {
        ...
        KeyPointsFilter::retainBest(keypoints, scoreType == MyORB::HARRIS_SCORE ? 2 * featuresNum : featuresNum);
    ...
    if( scoreType == MyORB::HARRIS_SCORE )
    {
#ifdef HAVE_OPENCL
        if( useOCL )
        {
...
void MyORB::detectAndCompute( InputArray _image, InputArray _mask,
                                 std::vector<KeyPoint>& keypoints,
                                 OutputArray _descriptors, bool useProvidedKeypoints,float _iso )
{
...
//    bool useOCL = MyOcl::useOpenCL();
    ...
    if( do_descriptors )
    {
        ...
        for( level = 0; level < nLevels; level++ )
        {
            ...
            // cv::blur(workingMat, workingMat, cv::Size(3, 3), cv::Point(-1, -1), cv::BORDER_DEFAULT);
            if(_iso > 4 && _iso <= 6){
                cv::blur(workingMat, workingMat, cv::Size(3, 3), cv::Point(-1, -1), cv::BORDER_DEFAULT);
            }
            else if(_iso > 6 && _iso <= 8){
                cv::blur(workingMat, workingMat, cv::Size(5, 5), cv::Point(-1, -1), cv::BORDER_DEFAULT);
            }
            else if(_iso > 8){
                cv::blur(workingMat, workingMat, cv::Size(7, 7), cv::Point(-1, -1), cv::BORDER_DEFAULT);
            }
        }
...
#endif
        {
            ...
            computeOrbDescriptors(imagePyramid, layerInfo, layerScale,
                                  keypoints, descriptors, pattern, dsize, wta_k,_iso);
        }
    }
}

Ptr<ORB_Base> ORB_Base::create(int nfeatures, float scaleFactor, int nlevels, int edgeThreshold,
                     int firstLevel, int wta_k, int scoreType, int patchSize, int fastThreshold)
{
    return makePtr<MyORB>(nfeatures, scaleFactor, nlevels, edgeThreshold,
