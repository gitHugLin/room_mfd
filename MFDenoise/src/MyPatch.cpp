...
//开始计时
void MyPatch::workBegin()
{
    work_begin = getTickCount();
}

//结束计时
void MyPatch::workEnd()
{
    work_end = getTickCount() - work_begin;
    cout<<"time = "<<work_end /((double)getTickFrequency() )* 1000.0<<"ms"<<endl;
}

void MyPatch::createPatch(const int nFeatures,const int nlevels,const int edgeThreshold,int _descriptorSize)
{
...
MyPatch::MyPatch(Point2f _origin,const Mat _queryImage,const Mat _trainImage )
{
    origin = _origin;
    qImage = _queryImage;
    tImage = _trainImage;
...
void MyPatch::setPatch(const Point2f _origin,const Mat _queryImage,const Mat _trainImage)
{
    if(!m_detector)
...
int MyPatch::detectKeypoints(void)
{
    int ret = 0;
...
void MyPatch::extractDescriptors(float _iso)
{
...
bool MyPatch::detectAndCompute(void)
{
    bool ret = false;
...
void MyPatch::sortMatcher(vector<DMatch>& matcher)
{
    std::sort(matcher.begin(),matcher.end(),copare);
    return;
}

void MyPatch::checkOffset(vector<DMatch>& matcher)
{
...
int MyPatch::getPointsMatchers(vector <Point2f> & _queryPoints,vector <Point2f> & _trainPoints,float _iso)
{
    mISO = _iso;
...
//保存特征点，在图上体现出来
void MyPatch::saveKeypoints(const Mat& image, const vector<KeyPoint>& keypoints, Mat& outImage)
{
...
//保存匹配的特征点，在图上体现出来
void MyPatch::saveMatches(const Mat& queryImage,
